...
test("capture, evidence and drill gates are visible",async({page})=>{
 await page.route("**/api/overview",route=>route.fulfill({json:base}));await page.goto("/");
...
test("uncertainty and unknowns remain visible",async({page})=>{
 ...
 await page.goto("/");await expect(page.getByText("INCONCLUSIVE",{exact:true})).toBeVisible();
