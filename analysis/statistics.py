...
def summarize(events: list[Opportunity]) -> dict[str, Any]:
    ...
    return {
        "numerator": successes, "denominator": known, "failures": failures,
        ...
        "credible_interval": list(map(float, beta.ppf(
            [0.025, 0.975], successes + 1, failures + 1
        ))) if known else None,
...
def categorical(actions: list[str], taxonomy: tuple[str, ...], prior: float = 1.0) -> dict[str, Any]:
    ...
    return {"n": len(actions), "prior": prior,
