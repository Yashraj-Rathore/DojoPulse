...
def analyze(
    source: Path, output: Path, metadata_path: Path | None = None,
    annotations_path: Path | None = None, templates_path: Path | None = None,
) -> dict[str, Any]:
    started = time.monotonic()
    report: dict[str, Any] = {
        "schema_version": "capture-report/1", "analysis_version": "local-pipeline/1",
        "capture_profile": profile()["id"], "game_build": None,
        "status": "VALIDATING", "opportunities": [], "observations": [],
        "issues": [], "automatic_gameplay_validated": False,
    }
    ...
    try:
        ...
        if templates_path:
            ...
            for entry in config["templates"]:
                ...
                for sample in samples["samples"]:
                    candidate = template_candidates(directory / sample["file"], template,
                                                    tuple(entry["region"]), entry["threshold"])
                    if candidate:
                        raw.append({"label": entry["label"], "timestamp_us": sample["timestamp_us"],
                                    "sample": sample["file"], **candidate})
        ...
        if annotations_path:
            ...
            if any(x["end_us"] > info["duration_seconds"] * 1_000_000
                   for x in annotations["examples"]):
        ...
        report["cost"] = {
            ...
            "source_bytes": info["bytes"], "derived_bytes": samples["stored_bytes"],
            "cloud_bytes_transferred": None, "cloud_cost_usd": None,
...
def main() -> None:
    ...
    parser.add_argument("--annotations", type=Path, help="Operator-reviewed sidecar; never client truth")
